...
```
GEO task proposal <domain-or-audit-file> [--tier basic|standard|premium] [--client-name "Name"] [--monthly EUR]
```

**Examples:**
```
GEO task proposal electron-srl.com
GEO task proposal electron-srl.com --tier standard --client-name "Electron Srl"
GEO task proposal ~/.geo-prospects/audits/electron-srl.com-2026-03-12.md
```
...
2. If not, suggest running `GEO task quick <domain>` first
...
3. Print confirmation:
   ...
   Next: Share the proposal file or run `GEO task report-pdf` for a visual version.
   ```
