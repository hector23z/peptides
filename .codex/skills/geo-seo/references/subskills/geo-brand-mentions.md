...
For each platform, use the web access tool to search and assess presence:
...
1. the web access tool: `https://en.wikipedia.org/wiki/[Brand_Name]` — check if the page loads (not a redirect to search)
2. the web access tool: `https://en.wikipedia.org/wiki/[Founder_Name]` for founder article
