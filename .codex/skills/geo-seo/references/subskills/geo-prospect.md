...
|---------|-------------|
| `GEO task prospect new <domain>` | Create new prospect (interactive prompts) |
| `GEO task prospect list` | Show all prospects with pipeline status |
| `GEO task prospect list <status>` | Filter: lead, qualified, proposal, won, lost |
| `GEO task prospect show <id-or-domain>` | Full prospect detail with history |
| `GEO task prospect audit <id-or-domain>` | Run quick GEO audit and save to prospect record |
| `GEO task prospect note <id-or-domain> "<text>"` | Add interaction note with timestamp |
| `GEO task prospect status <id-or-domain> <new-status>` | Move through pipeline |
| `GEO task prospect won <id-or-domain> <monthly-value>` | Mark as won, set contract value |
| `GEO task prospect lost <id-or-domain> "<reason>"` | Mark as lost with reason |
| `GEO task prospect pipeline` | Visual pipeline summary with revenue forecast |
...
6. Save to JSON file
7. Suggest next step: "Run `GEO task prospect audit electron-srl.com` to score this prospect"

### `GEO task prospect list`
...
### `GEO task prospect audit <id-or-domain>`

1. Run `GEO task quick <domain>` to get GEO snapshot score
...
6. If score < 55: suggest "Score indicates strong sales opportunity. Run `GEO task proposal <domain>` to generate proposal."

### `GEO task prospect note <id-or-domain> "<text>"`
...
### `GEO task prospect status <id-or-domain> <status>`
