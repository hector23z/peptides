...
**IMPORTANT:** the web access tool converts HTML to markdown and strips `<head>` content, which removes JSON-LD blocks. For schema detection, use the fetch_page.py script instead:
```bash
python3 <geo-seo-skill-root>/scripts/fetch_page.py <url> page
```
