...
- Use the web access tool to retrieve the target URL.
...
1. **YouTube**: Use the web access tool to search `site:youtube.com "brand name"` patterns. Check for official channel presence, video count, and engagement.
...
3. **Wikipedia (CRITICAL — use API check, not just web search)**:
   - **FIRST**, run the Wikipedia API directly via Bash to check definitively:
     ...
     "
     ```
   - **SECOND**, try the web access tool on `https://en.wikipedia.org/wiki/[Brand_Name]` directly to verify.
...
- If the web access tool fails for a platform check, note the failure and do not fabricate results.
